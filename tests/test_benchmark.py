...
def test_benchmark_runner_rejects_non_string_provenance_callables() -> None:
    ...
    for field_name in (
        ...
        values = {**common, field_name: object()}
        try:
            BenchmarkRunConfiguration(**values)
        except TypeError as exc:
            assert field_name in str(exc)
        else:
            raise AssertionError("non-string provenance callable fields must be rejected")


def test_benchmark_runner_rejects_whitespace_provenance_callables() -> None:
    common = {
        "benchmark_name": "provenance-contract-smoke",
        "episode_count": 1,
        "max_steps": 1,
    ...
    for field_name in (
        ...
        values = {**common, field_name: "   "}
        try:
            BenchmarkRunConfiguration(**values)
        except ValueError as exc:
            assert field_name in str(exc)
        else:
            raise AssertionError("whitespace provenance callable fields must be rejected")
...
def test_benchmark_runner_does_not_mask_episode_failure_with_cleanup_failure() -> None:
    ...
    try:
        ...
        raise AssertionError("episode failure should remain primary")
