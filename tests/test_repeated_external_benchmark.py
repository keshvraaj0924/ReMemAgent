...
def test_repeated_external_benchmarks_reject_empty_seeds() -> None:
    ...
    try:
        ...
        assert str(exc) == "seeds must contain at least one seed"
    else:
        raise AssertionError("empty repeated seed lists must be rejected")
...
def test_repeated_external_benchmarks_reject_duplicate_seeds() -> None:
    ...
    try:
        ...
        assert str(exc) == "seeds must be unique"
    else:
        raise AssertionError("duplicate repeated seeds must be rejected")
