...
The probe is not benchmark data. It is a fail-fast integration check and its output is not included in measured reports. The temporary environment created for the probe is always closed. If the environment or policy probe fails, cleanup is attempted without replacing the original failure; if probing succeeds, a cleanup failure remains visible to the caller.
