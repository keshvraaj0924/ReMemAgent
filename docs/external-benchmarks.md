...
The concrete WebShop factory normalizes the observation mode and Gym environment identifier once at factory creation. Both concrete factories scope Python's module-level RNG **and, when NumPy is installed, NumPy's module-level RNG** to the requested episode seed during environment construction and reset, then restore the caller's exact previous RNG state. The seeded wrappers reject an explicit `seed` argument on reset because ReMemAgent owns that seed contract. This matches the upstream WebShop implementation, which imports both Python `random` and NumPy for environment behavior.
