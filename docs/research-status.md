...
The normalized `StepResult` contract now validates adapter output at the common environment boundary. Observations must be strings, rewards must be finite numeric values, terminal flags must be actual booleans, and metadata must be a dictionary. This prevents NaN/infinite rewards and malformed terminal state from reaching benchmark aggregation or training integrations. The validation is dependency-free and applies equally to ALFWorld, WebShop, and local adapters.
