...
The repository now provides a multi-seed runner that gives each seed an isolated random generator and preserves per-run case and experiment fingerprints. It does not calculate or imply statistical significance, and it does not fabricate or infer missing experimental evidence.
