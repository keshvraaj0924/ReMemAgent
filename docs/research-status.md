...
The training integration layer can now persist validated GRPO or verl batches as deterministic JSON Lines. Writers create parent directories, preserve row ordering, sort JSON keys, and avoid introducing tokenizer or trainer dependencies. This provides an explicit offline artifact boundary for later mapping into a concrete GRPO/verl dataset schema without pretending that a JSONL file itself is a trainer integration.
